...
{
    private $url = '/master/students';
    private $pathViews = 'master.students.';
    ...
    public function save(Request $request)
    {
        ...
        $id = $request['id'];
        $rules = [
            'name' => 'required|max:225|unique:students,name,' . $id . ',student_id',
        ];
    ...
    public function dataTable(Request $request)
    {
        ...
        $total = Student::count();
        if ($search) {
            $filteredTotal = Student::where(db::raw("lower(name)"), "like", "%$search%")->count();
        } else {
            $filteredTotal = $total;
        }

        $data = Student::where(db::raw("lower(name)"), "like", "%$search%")
            ->offset($offset)
            ->limit($limit)
        ...
        $result = [];
        foreach ($data as $dt) {
            $urlEdit = url($this->url . '/' . $dt->student_id . '/edit');
            $actionData = '<a href="' . $urlEdit . '" class="btn btn-sm btn-info" title="Edit">Edit</a>';
            $actionData .= '<form action="' . url($this->url . '/' . $dt->student_id . '/delete') . '" method="post" style="display: inline;">
